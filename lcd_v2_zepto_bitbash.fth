...
: >> rshift ;
: << lshift ;
: ROL ( rotate left, top bit comes bottom bit... ) dup $80000000 and if 1 << 1+ else 1 << then ;
...
: SPIWR 8 0 do ROL DUP 1 and lcd_din PIN! SPICLK LOOP ;
...
\
\ Emulate Zeptoforth interface
\

\ bit bashed SPI interface
: >spi ( data spi -- )
    drop SPIWB
;
: spi-pin ( spi pin -- )
